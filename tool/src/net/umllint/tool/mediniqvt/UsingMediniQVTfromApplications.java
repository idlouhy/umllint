 ...
 */
public class UsingMediniQVTfromApplications extends UsingMediniQVTBase {
	 ...
	 * 
	 * @param outputStream
	 *            an {@link java.io.OutputStream} for logging
	 */
	...
	/**
	 * Example usage of the classes {@link UsingMediniQVTfromApplications}.
	...
	public void launch() {
		...
		//Resource resource1 = this.getResource("example/source.xmi");
		Resource resource1 = this.getResource(getXmiSource());
		Resource resource2 = this.getResource(getXmiTarget());
    ...
    File fSource = new File(getXmiSource());
    File fTarget = new File(getXmiTarget());
    ...
    resource2 = this.getResource(fTarget.getAbsolutePath());
		...
		try {
			qvtRuleSet = new FileReader(getQvtScript()); // TODO: adjust the filename!
		...
		// the QVT file!
		String transformation = "lint"; // TODO: adjust
